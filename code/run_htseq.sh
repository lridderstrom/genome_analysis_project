...
run_htseq_counts() {
  ...
  for BAM in $MAPPING_DIR/$LOCATION/*.bam; do
    ...
    GFF_FILE=$ANNOTATED_DIR/$BIN_NAME/${BIN_NAME}_clean.gff

    if [[ -f "$GFF_CLEAN" ]]; then
      echo "HTSeq-count: $BIN_NAME @ $LOCATION"
      htseq-count -f bam -r pos -s no -i ID "$BAM" "$GFF_CLEAN" > "$COUNTS_DIR/$LOCATION/${BIN_NAME}_counts.txt"
    else
      echo "⚠️  Can't find the .gff file for $BIN_NAME – skips it."
    fi
  done
}
